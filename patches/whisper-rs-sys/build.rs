...
fn main() {
    ...
    if env::var("WHISPER_DONT_GENERATE_BINDINGS").is_ok() {
        write_bindings_without_layout_tests(out.join("bindings.rs"), target.contains("windows"));
    } else if target.contains("windows") {
        // MSVC/libclang bindgen often emits opaque whisper_full_params on Windows.
        write_bindings_without_layout_tests(out.join("bindings.rs"), true);
    } else {
...
/// Bundled bindings include Linux-only layout assertions; strip them for Windows MSVC.
fn write_bindings_without_layout_tests(dest: PathBuf, windows: bool) {
    let content =
        std::fs::read_to_string("src/bindings.rs").expect("Failed to read src/bindings.rs");
    let mut out = String::new();
    let mut skipping = false;
    let mut skipping_test = false;
    ...
    std::fs::write(dest, out).expect("Failed to write bindings.rs");
}
